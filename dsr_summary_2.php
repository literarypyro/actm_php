...
?>
<meta http-equiv="refresh" content="60;url=dsr_summary_2.php" />
...
if($extAvNM>0){ 
	if(isset($_GET['ext'])){ 
		echo "| <a href='dsr_summary_2.php'>Satellite</a> "; 
	}
	else {
		echo "| <a href='dsr_summary_2.php?ext=Y'>Extension</a> "; 
...
<tr class='header'>
	<th  style='border:1px solid gray'>GROSS SALES/SJ</th>
...
<tr class='grid'>
	...
	<td style='border:1px solid gray' align=right>&nbsp;</td>

	</tr>	
<tr class='grid'>
	<th style='border:1px solid gray'>Concessionary</th>
	<td style='border:1px solid gray' align=right>&nbsp;</td>
...
//	$sql2="select sum(sj_amount+sv_amount) as refund from refund inner join control_remittance on refund.control_id=control_remittance.control_id where remit_log='".$log_id."'";
	$sql2="select (sj_amount+tvm) as refund,tvm,unit from refund inner join control_remittance on refund.control_id=control_remittance.control_id where remit_log='".$log_id."'";
	...
	$adjustmentSQL="select * from control_sales_amount inner join control_remittance on control_sales_amount.control_id=control_remittance.control_id where remit_log='".$log_id."' and station in ('".$station."','".$extensionStamp."')";
...
//		$unpaid_shortage+=$row2['shortage'];
	...
	if($discrepancyNM>0){
		...
		$paid_shortage+=$discrepancyRow['sjt_net_revenue'];
	...
	$sql="select * from logbook where date='".$dsrDate."' and station in ('".$station."','".$extensionStamp."') order by field(revenue,'open','close'),field(shift,3,1,2)";
	$rs=$db->query($sql);
	...
	$subtotal=$gross_sales_sj+$cash_beginning+$xsjt_sales['sjt'];	

//	$subtotal_svc=$svc_net_revenue+$svc_add_value+$svc_issuance_fee+$issuance_unreg;

	$subtotal+=$svc_add_value+$svc_issuance_fee;
	...
	$overage=$overage;
//	$refund_offset=abs(($refund*1)-($tvm_ref*1));

	$refund_offset=abs(($tvm_overage*1)-($ref_val*1));
	

	if($ref_val<$tvm_overage){ $unpaid_shortage=$refund_offset; $overage=0;  }
	else if($ref_val>$tvm_overage){ $overage=$refund_offset; $unpaid_shortage=0;  }
	else {$overage=0;$unpaid_shortage=0; }
	
	$overage+=$ntvm_overage*1;
...
<tr class='grid'>
	<th style='border:1px solid gray'>Fixed Fund</th>
	<td style='border:1px solid gray' align=right><?php echo number_format($fixed_fund*1,2); ?></td>
...
<tr class='grid'>
	<th style='border:1px solid gray'>GROSS SALES/SJ</th>
	<td style='border:1px solid gray' align=right><?php echo number_format($gross_sales_sj*1+$xsjt_sales['sjt']*1,2); ?></td>
