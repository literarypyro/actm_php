...
$filename="treasury forms/DSR_revision proposed.xls";
...
//$sql="select * from logbook where date='".$dsrDate."' and station in ('".$station."','".$extS."') order by field(revenue,'open','close'),field(shift,3,1,2)";
...
//			$allocationSQL="select * from control_sold where control_id in (SELECT control_id FROM control_remittance where remit_log='".$log_id."' and station='".$stationStamp."' and remit_ticket_seller='".$row2['remit_ticket_seller']."')";
			...
			$allocationSQL="select * from control_sold inner join control_remittance on control_sold.control_id=control_remittance.control_id where remit_log='".$log_id."' and station='".$stationStamp."' and remit_ticket_seller='".$row2['remit_ticket_seller']."' and unit='".$row2['unit']."'";
			...
			$adjustmentSQL="select * from control_sales_amount inner join control_remittance on control_sales_amount.control_id=control_remittance.control_id where remit_log='".$log_id."' and station='".$stationStamp."' and remit_ticket_seller='".$row2['remit_ticket_seller']."' and unit='".$row2['unit']."'";
		...
		//		$svtIssuance+=$ticket_amount['svc']['issuance_fee']*1;
			...
			$fareSQL="select * from fare_adjustment inner join control_remittance on fare_adjustment.control_id=control_remittance.control_id where remit_log='".$log_id."' and station='".$stationStamp."' and remit_ticket_seller='".$row2['remit_ticket_seller']."' and unit='".$row2['unit']."'";
			...
			$unregSQL="select * from unreg_sale inner join control_remittance on unreg_sale.control_id=control_remittance.control_id where remit_log='".$log_id."' and station='".$stationStamp."' and remit_ticket_seller='".$row2['remit_ticket_seller']."' and unit='".$row2['unit']."'";
			...
			$discountSQL="select * from discount inner join control_remittance on discount.control_id=control_remittance.control_id where remit_log='".$log_id."' and station='".$stationStamp."' and remit_ticket_seller='".$row2['remit_ticket_seller']."' and unit='".$row2['unit']."'";
			...
			$refundSQL="select * from refund inner join control_remittance on refund.control_id=control_remittance.control_id where remit_log='".$log_id."' and station='".$stationStamp."' and remit_ticket_seller='".$row2['remit_ticket_seller']."' and unit='".$row2['unit']."'";
			...
			for($m=0;$m<$refundNM;$m++){
				...
				$sv_ref_amount+=$sv_r_amount;
			}

			$cashSQL="select unit,sum(if(discrepancy.type='overage',amount,0)) as overage,sum(if(discrepancy.type='shortage',amount,0)) as unpaid_shortage from discrepancy inner join cash_transfer on discrepancy.transaction_id=cash_transfer.transaction_id where discrepancy.log_id='".$log_id."' and discrepancy.ticket_seller='".$row2['remit_ticket_seller']."' and unit='".$row2['unit']."' and cash_transfer.station='".$stationStamp."'";
			...
			$discrepancySQL="SELECT * FROM transaction inner join cash_transfer on transaction.transaction_id=cash_transfer.transaction_id where transaction_type='shortage' and transaction.log_id='".$log_id."' and cash_transfer.station in ('".$stationStamp."','".$extS."') and cash_transfer.ticket_seller='".$row2['remit_ticket_seller']."' and unit='".$row2['unit']."'";
			...
			if($discrepancyNM>0){
				for($aa=0;$aa<$discrepancyNM;$aa++){
					...
					$paid_shortage+=$discrepancyRow['sjt_net_revenue']+$discrepancyRow['total'];
				}
			}
...
//			$unsoldSQL="select type,sum(loose_good) as ticket_sum,sum(loose_defective) as ticket_sum2 from control_unsold where control_id in (SELECT control_id FROM control_remittance where remit_log='".$log_id."' and station='".$stationStamp."' and remit_ticket_seller='".$row2['remit_ticket_seller']."') group by type";
			...
			$unsoldSQL="select sum(sj_defective) as sj_defective_sum,sum(sj_unconfirmed) as sj_unconfirmed_sum from control_defective where control_id='".$row2['control_id']."'";
	...
	//			if($unsoldRow['type']=='sjt'){
					//$sjtLoose=$unsoldRow['ticket_sum'];	
					//$sjtDefective=$unsoldRow['sj_defective_sum'];
					$sjtUncertain=$unsoldRow['sj_unconfirmed_sum'];
...
//				}
			...
			$discrepancyTicketSQL="select *,sum(amount) as new_amount from discrepancy_ticket inner join control_remittance on transaction_id=control_id where remit_log='".$log_id."' and control_remittance.ticket_seller='".$row2['remit_ticket_seller']."' and unit='".$row2['unit']."' and station='".$stationStamp."' group by ticket_type";
...
//				addContent(setRange("Q".$rowCount,"Q".$rowCount),$excel,($sv_discount),"true",$ExWs);
				...
				*/
				...
				addContent(setRange("Z".$rowCount,"Z".$rowCount),$excel,$sjtUncertain,"true",$ExWs);


				addContent(setRange("AA".$rowCount,"AA".$rowCount),$excel,$svtDefective,"true",$ExWs);
				/*
...
addContent(setRange($prefix[88].$rowCount,$prefix[88].$rowCount),$excel,$gridLabel,"true",$ExWs);


	$grand_issuance="=I".$rowCount;
	$grand_add_value="=J".$rowCount;
...
//	$sql2="select sum(ot) as ot from control_cash where control_id in (SELECT control_id FROM control_remittance where remit_log='".$log_id."' and station='".$stationStamp."') group by type";
	...
	$sql2="select * from control_defective inner join control_remittance on control_defective.control_id=control_remittance.control_id  where remit_log='".$log_id."' and station='".$stationStamp."'";
	$rs2=$db->query($sql2);
	$nm2=$rs2->num_rows;	
	for($k=0;$k<$nm2;$k++){
		$row2=$rs2->fetch_assoc();
		$sjt_defective+=$row2['sj_defective'];
		$sjt_unconfirmed+=$row2['sj_unconfirmed'];
	}	
...
$netSales=$grandTotal-$deductionsTotal;
	...
	*/
	...
	addContent(setRange("B35","C35"),$excel,"=L".$grandRowCount,"true",$ExWs);
	addContent(setRange("B36","C36"),$excel,"=K".$grandRowCount,"true",$ExWs);

	//addContent(setRange("B37","C37"),$excel,$tvm_refund,"true",$ExWs);
	addContent(setRange("B37","C37"),$excel,"=O".$grandRowCount,"true",$ExWs);

	
	
	//addContent(setRange("C39","C39"),$excel,"=sum(C33:C38)","true",$ExWs);
//	addContent(setRange("B39","C39"),$excel,"=Q".$grandRowCount."+S".$grandRowCount,"true",$ExWs);
...
for($i=0;$i<$nm;$i++){
	...
	$sql2="select sum(amount) as ticket_sum,ticket_type,type from discrepancy_ticket where log_id='".$log_id."' group by ticket_type,type";
	$rs2=$db->query($sql2);
	$nm2=$rs2->num_rows;		
...
$svd_grand_total=$svd_subtotal-$svd_physically_defective-$svd_deduction+$svd_discrep;
	...
	addContent(setRange("J36","K36"),$excel,$svt_additional_amount,"true",$ExWs);
	
	addContent(setRange("H38","I38"),$excel,($sold_ticket_1['sjt']['reg']+$sold_ticket_1['sjt']['disc']),"true",$ExWs);
	addContent(setRange("J38","K38"),$excel,$sold_ticket_1['svc']['reg'],"true",$ExWs);
	
	addContent(setRange("H39","I39"),$excel,$sjt_physically_defective,"true",$ExWs);
	addContent(setRange("J39","K39"),$excel,$svt_physically_defective,"true",$ExWs);
	
	addContent(setRange("H40","I40"),$excel,$sjt_defective,"true",$ExWs);
	addContent(setRange("J40","K40"),$excel,$svt_defective,"true",$ExWs);

	addContent(setRange("H41","I41"),$excel,$sjt_unconfirmed,"true",$ExWs);
	addContent(setRange("J41","K41"),$excel,$svt_unconfirmed,"true",$ExWs);


	addContent(setRange("H42","I42"),$excel,(($sjt_overage)*1),"true",$ExWs);
	addContent(setRange("J42","K42"),$excel,(($svt_overage)*1),"true",$ExWs);

	addContent(setRange("H43","I43"),$excel,(($sjt_shortage)*1),"true",$ExWs);
	addContent(setRange("J43","K43"),$excel,(($svt_shortage)*1),"true",$ExWs);
...
//	$sql2="select sum(sj+sv) as unreg_sale from unreg_sale inner join control_remittance on unreg_sale.control_id=control_remittance.control_id where remit_log='".$log_id."'";
	...
	$sql2="select sum(sj_amount+tvm) as refund,unit from refund inner join control_remittance on refund.control_id=control_remittance.control_id where remit_log='".$log_id."'";
	$rs2=$db->query($sql2);
	$nm2=$rs2->num_rows;	
	for($k=0;$k<$nm2;$k++){
		$row2=$rs2->fetch_assoc();
		if($row2['unit']=="TIM1"){ $tvm_ref=$row2['refund']; } 
		$refund+=$row2['refund'];
	}	
...
$refund_offset=abs(($refund*1)-($tvm_ref));

$grand_add_value=$svc_add_value;
$grand_issuance=$svc_issuance_fee;
...
*/
	...
	$sql2="select (if(discrepancy.type='overage',amount,0)) as overage,(if(discrepancy.type='shortage',amount,0)) as shortage,unit from discrepancy inner join cash_transfer on discrepancy.transaction_id=cash_transfer.transaction_id where discrepancy.log_id='".$log_id."'";
...
//		$overage+=$row2['overage'];
		...
		if(($row2['unit']=="TIM1")||($row2['unit']=="TIM2")||($row2['unit']=="TIM3")){
			$tvm_overage+=$row2['overage']*1;
		}
		else {
			$ntvm_overage+=$row2['overage']*1;
		}
	...
	$sql2="select * from fixed_fund where station_id='".$station."'";
	$rs2=$db->query($sql2);
	$row2=$rs2->fetch_assoc();
	
	$fixed_fund=$row2['fixed_fund'];
	...
	$subtotal_svc=$svc_net_revenue+$svc_add_value+$svc_issuance_fee+$fixed_fund;
	...
	$refund_offset=abs(($tvm_overage-$sv_ref_amount)*1);
	
	if($tvm_overage<$sv_ref_amount){


		$unpaid_shortage=$refund_offset;
		$overage=0;


	}
	else if($tvm_overage>$sv_ref_amount){
		$overage=$refund_offset;
		$unpaid_shortage=0;
...
//	$overage=$refund_offset;
	...
	$sql="select * from logbook where date='".$dsrDate."' and station in ('".$stationStamp."','".$extensionStamp."') order by field(revenue,'open','close'),field(shift,3,1,2)";
	$rs=$db->query($sql);
	...
	if($nm>0){
		...
		for($i=0;$i<$nm;$i++){
			...
			$sql2="select * from control_sales_amount inner join control_remittance on control_sales_amount.control_id=control_remittance.control_id where  source_type='pos' and ticket_type='sjt' and remit_log='".$log_id."' and station='".$extensionStamp."'";
			$rs2=$db->query($sql2);
			$nm2=$rs2->num_rows;	
			...
			for($k=0;$k<$nm2;$k++){
				$row2=$rs2->fetch_assoc();
				$xsjt_sales[$row2['ticket_type']]+=$row2['amount']*1;		

					
			}
/*			
			$refundSQL="select * from refund inner join control_remittance on refund.control_id=control_remittance.control_id where remit_log='".$log_id."' and station='".$extensionStamp."'";
			$refundRS=$db->query($refundSQL);
			$refundNM=$refundRS->num_rows;
...
*/			
	...
	if($nm>0){
		...
		for($i=0;$i<$nm;$i++){
			...
			$refundSQL="select * from refund inner join control_remittance on refund.control_id=control_remittance.control_id where remit_log='".$row['id']."' and station='".$extensionStamp."'";
			$refundRS=$db->query($refundSQL);
			$refundNM=$refundRS->num_rows;
...
//				$sj_r_amount+=$refundRow['sj_amount']*1;
	...
	$unpaid_shortage+=$sv_ref_amount2;
	...
	if(isset($_GET['ext'])){
		...
		addContent(setRange("V32","Z32"),$excel,$cash_beginning,"true",$ExWs);


		//addContent(setRange("V32","Z32"),$excel,$svc_net_revenue,"true",$ExWs);

		addContent(setRange("V33","Z33"),$excel,$fixed_fund,"true",$ExWs);


		addContent(setRange("V34","Z34"),$excel,"=B39+".$xsjt_sales['sjt']*1,"true",$ExWs);

//		addContent(setRange("V35","Z35"),$excel,$svc_issuance_fee,"true",$ExWs);

//		addContent(setRange("V36","Z36"),$excel,$svc_add_value,"true",$ExWs);


		addContent(setRange("V35","Z35"),$excel,$grand_issuance,"true",$ExWs);

		addContent(setRange("V36","Z36"),$excel,$grand_add_value,"true",$ExWs);
...
//		addContent(setRange("V35","Z35"),$excel,"=I".$grandRowCount."+N".$grandRowCount,"true",$ExWs);
		...
		addContent(setRange("V39","Z39"),$excel,(($pnb_deposit_p['sjt']+$pnb_deposit_p['svc'])*1),"true",$ExWs);


		addContent(setRange("V40","Z40"),$excel,(($pnb_deposit_c['sjt']+$pnb_deposit_c['svc'])*1),"true",$ExWs);
		...
		addContent(setRange("V43","Z43"),$excel,$overage,"true",$ExWs);
		addContent(setRange("V41","Z41"),$excel,$unpaid_shortage,"true",$ExWs);
