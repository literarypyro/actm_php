...
//				'".$sjt_total."','".$sjd_total."','".$svt_total."','".$svd_total."')";
			...
			$sql2="select * from control_sales_amount where control_id='".$control_id."' and source_type='pos' and ticket_type='svc' and value_type='issuance_fee'";
			$rs2=$db->query($sql2);
			$nm2=$rs2->num_rows;
			
			if($nm2==0){
				$sql2="insert into control_sales_amount(control_id,source_type,ticket_type,value_type,amount) values ";
				$sql2.="('".$control_id."','pos','svc','issuance_fee','".($svt_total*20)."')";
				$rs2=$db->query($sql2);
			}	
			else {
				$row2=$rs2->fetch_assoc();
				$sql2="update control_sales_amount set amount='".($svt_total*20)."' where id='".$row2['id']."'";
				$rs2=$db->query($sql2);	
			}
	...
	//$record_rs=$record_db->query($record_sql);	
		...
		for($i=0;$i<count($tickets);$i++){			
			if(($_POST[$tickets[$i]."_unsold_a"]=="")&&($_POST[$tickets[$i]."_unsold_b"]=="")&&($_POST[$tickets[$i]."_unsold_c"]=="")){
				...
				$loose_uncertain=$_POST[$tickets[$i]."_unsold_d"];
				...
				if($nm==0){
					$sql="insert into control_unsold(control_id,source_type,type,sealed,loose_good,loose_defective,loose_uncertain,transaction_id) values ";
					$sql.="('".$control_id."','pos','".$tickets[$i]."','".$sealed."','".$loose_good."','".$loose_defective."','".$loose_uncertain."','".$transaction_id."')";

					$rs=$db->query($sql);

				}
				else {
					$sql="update control_unsold set sealed='".$sealed."', loose_good='".$loose_good."',loose_uncertain='".$loose_uncertain."', loose_defective='".$loose_defective."' where control_id='".$control_id."' and type='".$tickets[$i]."'";

					$rs=$db->query($sql);	
				}

				if($tickets[$i]=="sjt"){
					$sql="select * from control_defective where control_id='".$control_id."'";
					$rs=$db->query($sql);
					$nm=$rs->num_rows;
					
					if($nm==0){
						$sql="insert into control_defective(control_id,sj_defective,sj_unconfirmed) values ";
						$sql.="('".$control_id."','".$loose_defective."','".$loose_uncertain."')";

						$rs=$db->query($sql);

					}
					else {
						$sql="update control_defective set sj_unconfirmed='".$loose_uncertain."', sj_defective='".$loose_defective."' where control_id='".$control_id."'";
...
function computeSequence2(type,column,e,nextField){
//	computeSubTotal(column);
...
//	total+=document.getElementById('svd'+suffix).value*1;
	...
	document.getElementById('sold_total').value=total*1;
...
<th colspan=4  <?php if($_POST['type_transact']=="allocation"){ echo  "class='highlight'"; } else { echo "class='header'"; } ?> id='unsold_header' name='unsold_header'>Unsold/Excess</th>
...
for($i=0;$i<$nm;$i++){
	...
	$unsold[$row['type']]["loose_uncertain"]=$row['loose_uncertain'];
	...
	$total_unsold_c+=$unsold[$row['type']]['loose_uncertain'];

	$unsold[$row['type']]['total']=$row['sealed']*1+$row['loose_good']*1+$row['loose_defective']*1+$row['loose_uncertain']*1;
}
...
<td><input type=text size=5 name='svt_unsold_c' id='svt_unsold_c' onkeyup="computeSequence('svt','_unsold_c',event,'sjt_unsold_d')" onfocus='focusHeader("remittance")'   value='<?php echo $unsold["svt"]["loose_defective"]; ?>'  /></td>


<td><input type=text size=5 name='svt_unsold_d' id='svt_unsold_d' onkeyup="computeSequence('svt','_unsold_d',event,'sjt_unsold_d')" onfocus='focusHeader("remittance")'   value='<?php echo $unsold["svt"]["loose_uncertain"]; ?>'  /></td>


<td><input type=text name='svt_total' id='svt_total' onfocus='focusHeader("ticket_sold")' onkeyup="computeSequence2('svt','_total',event,'sjt_total')" value='<?php echo $sold_tickets["svc"]["reg"];?>' /></td>
...
<td rowspan=2><input type=text size=5 name='sjt_unsold_c' id='sjt_unsold_c' onkeyup="computeSequence('sjt','_unsold_c',event,'svt_unsold_d')"  onfocus='focusHeader("remittance")'   value='<?php echo $unsold["sjt"]["loose_defective"]; ?>' /></td>

<td rowspan=2><input type=text size=5 name='sjt_unsold_d' id='sjt_unsold_d' onkeyup="computeSequence('sjt','_unsold_d',event,'svt_unsold_d')"  onfocus='focusHeader("remittance")'   value='<?php echo $unsold["sjt"]["loose_uncertain"]; ?>' /></td>


<td class='<?php echo $class; ?>'><input type=text  name='sjt_total' id='sjt_total' onfocus='focusHeader("ticket_sold")' onkeyup="computeSequence2('sjt','_total',event,'sjd_total')" value='<?php echo $sold_tickets["sjt"]["reg"];?>' /></td>
...
<tr>
<td>SJD</td>
<td class='<?php echo $class; ?>' ><input type=text name='sjd_total' id='sjd_total'  onkeyup="computeSequence('sjd','_total',event,'svt_total')" value='<?php echo $sold_tickets["sjt"]["disc"];?>' onfocus='focusHeader("ticket_sold")'  /></td>
...
<td><input type=text size=5 name='total_unsold_d' id='total_unsold_d'  value='<?php echo $total_unsold_d; ?>' /></td>
...
<tr class='grid'><th>Unreg Sale</th><td><input type=text name='unreg_sj' id='unreg_sj' onkeyup='computeRemittance()' value='<?php echo $sj_unreg; ?>'   /></td></tr>

<!--
<tr class='grid'><th>Lost (SVC)</th><td><input type=text name='unreg_sv' id='unreg_sv' onkeyup='computeRemittance()'   /></td></tr>
-->
